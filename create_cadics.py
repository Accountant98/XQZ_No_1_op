...
def create_cadics(case, market, powertrain, car):
    ...
    for group in dic_group_karenhyo12.keys():
        ...
        common_option,dic_kep=check_option(data_karenhyo2,file_spec)
            ...
            if len(list_cadics_filter) > 0:
                for cadics_no in list_cadics_filter:
                    list_dic_records = pick_car(data_karenhyo2, dict_except_config, cadics_no, adddress_config,
                                                ...
                                                dict_grade,common_option,max_car,dict_optioncode,flag_all,dic_kep)
...
def pick_car(data_karenhyo, dict_except_config, cadic_no, adddress_config, group, lot, data_spec, 
            infor_fix,address_zone,dict_grade,common_option,max_car,dict_optioncode,flag_all,dic_kep):
    ...
    if len(list_dict)==0:
        ...
        dic_data[131+max_car]=comment_nashi(col_opt_pick,row_opt,row_item,row_class,dic_kep)
        return [dic_data]
    else:
        return list_dict


def comment_nashi(col_opt_pick,row_opt,row_item,row_class,dic_kep):  
    dic_opt = {}
...
# a=create_cadics("CASE1.5","US","EV","YYYY")